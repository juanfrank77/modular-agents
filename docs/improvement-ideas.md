...
- **~~⭐ Declarative schedules in `BaseAgent`.~~** — **DONE (2026-07-24)**: Added `SCHEDULES` class-level attribute (`[("task_name", "cron_expr"), ...]`). `BaseAgent.register_schedules()` iterates over it and calls `scheduler.add_cron_job()` for each entry, centralizing the `chat_ids[0] if ... else ""` pattern. Removed boilerplate from BusinessAgent, DevOpsAgent, WellbeingAgent, ProjectsAgent, and EchoAgent.
