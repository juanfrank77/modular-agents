...
Quick wins doable in an afternoon: echo debug-gate, declarative `SCHEDULES` on BaseAgent, ~~composio-anthropic in requirements~~ (done), ~~CLI @agent parsing~~ (done), ~~time.monotonic() in rate limiter~~ (done), ~~unencrypted-DB warning~~ (done), ~~StartLimitBurst → [Unit]~~ (done), ~~attempts_remaining() accessor~~ (done), ~~RUNBOOK grep fix~~ (done), ~~duplicate Telegram handler removal~~ (done), §2 (LLM layer) fully closed, §8's core unit-test suite gap closed.
