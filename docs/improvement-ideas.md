...
- **No way to clear a pairing lockout.** — **DONE (2026-07-19)**: `PairingManager.MAX_FAILED_ATTEMPTS` (5) locked a chat_id permanently once persistence landed (§1.4) — `is_locked()` now survives restarts, and `delete_failed_attempts()` is only ever called from the success branch of `try_pair()`, which is unreachable once locked (`core/safety.py`). Added `PairingManager.unlock()` method and `POST /admin/unlock` endpoint (HTTP) to clear lockouts. Telegram interface has no equivalent unlock path yet.
...
- **Pairing code comparison is not constant-time.** — **NEW (2026-07-30)**: `PairingManager.try_pair()` (`core/safety.py:110`) uses `text.strip() == self._token`, and `interfaces/http.py` (`/pair` and admin endpoints) uses `req.code.strip() != self._safety.pairing.code`. Both are vulnerable to timing side-channels. A local-network attacker could measure response times to guess the code character-by-character. Should use `secrets.compare_digest` (or `hmac.compare_digest`) after stripping and case-normalizing.
...
- **Skill/solution matching is naive bag-of-words** - **PARTIAL (2026-07-25)**: (`skill_loader.py:57`, `memory._get_relevant_solutions`): no stemming ("meeting" ≠ "meetings"), no stopwords. The bag-of-words matching itself remains; file-content caching is not yet implemented — `_get_relevant_solutions` still re-reads every `.md` file per message. Add caching; consider embeddings when the library grows.
...
- `~~/model mutates the global default for everyone~~` — **DONE (2026-07-25)**: scoped per-chat rather than per-agent — per-agent runtime overrides would have duplicated the `<AGENT>_AGENT_MODEL` env vars item 2.3 already shipped. New `chat_model_map` table (`core/state_store.py`), `MessageBus.get_chat_model()`/`set_chat_model()`/`clear_chat_model()` (mirrors the `chat_agent_map` persistence pattern), and `BaseAgent.resolve_model(chat_id)` giving precedence: chat override → agent's env var → global default. `/model` in `telegram.py` now calls `bus.set_chat_model()` instead of mutating `settings.default_model`; added `/model reset` to clear the override. Note: `/model` is currently Telegram-only; no equivalent HTTP endpoint exists yet. Bundled with the item below since both trace back to the same routing ambiguity.
...
- **⭐ HTTP interface security and functional gaps — unreported bugs (2026-07-30).** Investigation of `interfaces/http.py` and its supporting `core/notifier.py` / `core/safety.py` paths found issues beyond the items already closed in §3 and §6. Current tests cover session persistence, admin endpoints, rate-limit caps, and tag parsing; the message/SSE paths and pairing-comparison paths are untested.
  - **Critical**: `/pair` and admin endpoints compare the pairing code with non-constant-time `!=` / `==` (also see §3). Brute-force can be accelerated by timing.
  - **Critical**: `/pair` invokes the per-IP rate limiter *after* validating the code. Wrong-code requests return 403 before the limiter is touched, so the rate limit does not slow brute-force attempts.
