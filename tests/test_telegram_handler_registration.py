...
class TestRegisterHandlers:
    ...
    def test_all_expected_handlers_are_registered(self):
        ...
        # 8 handlers: message, voice, callback, model, planmode, agents, unlock, command
        assert fake_app.add_handler.call_count == 8

        callbacks = {
            getattr(call.args[0], "callback", None)
            for call in fake_app.add_handler.call_args_list
        }
        assert telegram._on_message in callbacks
